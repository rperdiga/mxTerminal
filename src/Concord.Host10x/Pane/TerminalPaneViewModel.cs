    ...
    private async void HandleSaveSettings(SaveSettingsPayload p)
    {
        try
        {
            ...
            if (newMcpServerEnabled)
            {
                ...
                Concord.Host10x.Host10xEntry.Catalog?.SetFamilyEnabled(Terminal.Mcp.ToolFamily.UiActions, newStudioProActions);
                Concord.Host10x.Host10xEntry.Catalog?.SetFamilyEnabled(Terminal.Mcp.ToolFamily.Maia, maiaEnabled);
