...
**Current version: 6.0.0** ([CHANGELOG](./CHANGELOG.md)) — **Mac support + merged-shim deploy.** The runtime-shim architecture introduced in 5.1.0-alpha.1 now loads cleanly on **Studio Pro on macOS** (10.24.13 + 11.10) from the same `.mxmodule` artifact that already works on Windows. Root cause was `ConcordHostLoadContext`'s eager `AssemblyDependencyResolver` constructor invoking native `corehost_resolve_component_dependencies`, which Studio Pro's macOS embedded-host launcher can't satisfy (`InvalidArgFailure -2147450750`). Fix removes the resolver entirely and adds a `LoadUnmanagedDll` override that probes `runtimes/<rid>/native/` with RID fallback — resolution is now deterministic regardless of how .NET was hosted. Same `extensions/Concord/` install path works on all four supported targets (Windows + Mac × Studio Pro 10.24.13 + 11.10). 361 tests passing on both platforms.

Concord is a Mendix Studio Pro extension (Windows and macOS) supporting **Studio Pro 10.24.13 through 11.x** with full feature parity across both versions, that embeds a tabbed terminal as a dockable pane. The pane is the workspace where you run **Claude Code**, **Codex**, or **GitHub Copilot CLI** — and they talk directly to:
