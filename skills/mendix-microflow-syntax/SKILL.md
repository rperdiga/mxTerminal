...
description: Use when writing a Mendix microflow expression (any non-retrieve activity — change, decision, calculate, validate, etc.) or an XPath constraint (retrieve-from-database action). Picks the right syntax based on context. Includes full operator and function references for both, with examples and anti-patterns. Trigger any time the model needs to author Mendix syntax inside a microflow.
---

## Tools in this environment

- `ped_*` → `mcp__mendix-studio-pro__ped_*` (preferred — for any read/write of microflow content).
