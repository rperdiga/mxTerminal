...
| **Maia integration** *(Windows)* | 6 MCP tools — `maia__send`, `maia__status`, `maia__wait`, `maia__ask`, `maia__reset`, `maia__force_tier` — programmatic access to Studio Pro's in-IDE AI assistant |
| **7 bundled Mendix skill packs** | Installed into your project's `.claude/skills/` and `.github/skills/` (and `.codex/skills/` if you opt in) — prescriptive playbooks that teach the CLIs how to drive Mendix correctly |
| **Always-loaded build rules** *(Claude Code, new in 4.1.4)* | A project-level `concord-build-rules.md` that auto-loads into every Claude Code session in the project (via a managed block in your `CLAUDE.md`). Governs *how* the agent works — tool hierarchy, page-via-Maia doctrine, layout-first for branded apps, sibling-theme-module pattern, the named failure modes to guard against, three-part verification gate. Plus a `.claude/rules/project/` folder for your own project-specific additions that survives Concord upgrades |
| **Auto-wired CLI configs** | `.mcp.json` (Claude Code + Copilot CLI) and `~/.codex/config.toml` (Codex, opt-in) point at both MCP servers |
