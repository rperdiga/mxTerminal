...
description: Use when creating or modifying a Mendix page on macOS. Includes the widget catalog (LayoutGrid, DataView, ActionButton, TextBox, TextArea, DatePicker, CheckBox, RadioButtonGroup, DynamicText, DivContainer, TabContainer, Datagrid, etc.), the page-parameter rules, the data-source patterns, the validation checklist, and the macOS hand-off pattern (since `pg_read_page` / `pg_write_page` are not exposed and Maia is not reachable from a Mac CLI, page writes go through the human user pasting into Maia in Studio Pro). Trigger when the user asks to create a new page, modify an existing page, add a widget, change a layout, or wire a button to a microflow.
---

## Tools in this environment (macOS)

- `pg_read_page`, `pg_write_page` → **NOT exposed.** Studio Pro's MCP does not expose them, and Maia is **not reachable from a Mac CLI** (Concord's `maia__*` bridge is Windows-only — Studio Pro on Mac uses WKWebView, which can't be driven externally without host opt-in). All page writes must be performed by the human user pasting into Maia in Studio Pro themselves.
- `ped_check_errors`, `ped_create_document`, `ped_read_document` → `mcp__mendix-studio-pro__ped_*`. Used to verify pages after the user reports the write is done, and to create the page shell when CustomWidgets are involved.
- `mcp__concord-mcp__refresh_project` — refresh Studio Pro after a page write.

## macOS hand-off pattern

Because `pg_*` tools are not exposed and `maia__*` is Windows-only, every page read or write must be handed off to the human user. The fixed recipe:

1. **Build the JSON locally.** Use the widget catalog below to construct the full minified `pg_write_page` JSON yourself. Do not improvise — the JSON shape is what Maia expects, byte-for-byte.
2. **Hand off to the user.** Print **exactly** this block, with the placeholders filled in:

   ````
   I cannot reach Maia from a Mac CLI, so I need you to paste this into Maia in Studio Pro.

   1. In Studio Pro on Mac, open the **Maia** panel (right side).
   2. Make sure Maia is set to a model that supports tool use (e.g. Claude / GPT-4-class), and that `pg_write_page` is available in Maia's tool list.
   3. Copy the prompt below, paste it into Maia, and send.
   4. Wait for Maia to report success (or list errors).
   5. Reply here with **`done`** if Maia succeeded, or paste Maia's error output verbatim.
   ...
   Then **stop and wait for the user's reply.** Do not poll, do not retry, do not assume the write happened.

3. **Verify directly.** Once the user confirms `done`, call `mcp__mendix-studio-pro__ped_check_errors` yourself with `documentType: Pages$Page documentName: <ModuleName>.<PageName>` and again with `documentType: DomainModels$DomainModel documentName: <ModuleName>`. Do not trust the user's self-report — verify with the tool. (Maia may report success while leaving errors; only `ped_check_errors` is authoritative.)
4. **Retry on errors.** If errors are reported, rebuild the JSON addressing the specific errors and repeat steps 2–3. Maximum two retry attempts. After the second failed attempt, surface the JSON, the user's last report, and the `ped_check_errors` output to the user and stop.
...
2. Hand off to the user per the macOS recipe above to write the shell.
3. Tell the user: "After Maia writes the shell, please drag the `<widget name>` widget into the `<placeholder name>` container in Studio Pro — `ped_*` and `pg_*` cannot insert custom widgets reliably, and Maia's `pg_write_page` is unreliable for CustomWidgets too."
